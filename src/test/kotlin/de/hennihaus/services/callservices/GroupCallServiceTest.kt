...
import de.hennihaus.services.callservices.resources.GroupPaths
        ...
        fun `should return a list of groups and build call correctly`() = runBlocking {
            ...
            val engine = MockEngineBuilder.getMockEngine(
                ...
                assertions = {
                    ...
                    it.url shouldBe Url(
                        urlString = "${config.protocol}://${config.host}:${config.port}${GroupPaths.GROUPS_PATH}"
                    )
        ...
        fun `should return an updated group and build call correctly`() = runBlocking {
            ...
            val engine = MockEngineBuilder.getMockEngine(
                ...
                assertions = {
                    ...
                    it.url shouldBe Url(
                        urlString = """
                            ${config.protocol}://${config.host}:${config.port}${GroupPaths.GROUPS_PATH}/${group.id}
                        """.trimIndent()
                    )
                    it.headers[HttpHeaders.Accept] shouldBe "${ContentType.Application.Json}"
            ...
            result shouldBe GroupObjectMother.getFirstGroup()
        }
